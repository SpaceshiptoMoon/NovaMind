...
- **站内通知**: 系统事件、协作消息统一收口，支持已读/未读/批量已读
- **邮件通知**: 可选 SMTP 邮件推送
- **偏好设置**: 用户自定义通知接收偏好

### ClawMate 终端 — 浏览器内的 AI 终端

- **Shell 会话**: 用户级隔离终端，执行命令、读写文件
- **安全沙箱**: 命令黑名单 + 危险模式检测 + 写路径白名单 + 输出截断 + 空闲清理
- **AI 对话**: SSE 流式对话，自动管理会话与记忆
