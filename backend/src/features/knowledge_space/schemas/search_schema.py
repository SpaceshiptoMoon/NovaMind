...
"""
    ...
    # 嵌套配置
    weights: WeightConfig | None = Field(
        default=None,
    ...
    llm: SearchLLMConfig | None = Field(
        default=None,
        description="LLM 回答配置（为空则不生成回答）",
    )
    query_rewrite: QueryRewriteConfig | None = Field(
        default=None,
...
class SearchResult(BaseModel):
    ...
    questions: list[str] | None = Field(None, description="该分块预生成的假设性问题列表")

    # 元数据
    metadata: dict[str, Any] | None = Field(None, description="分块元数据")
    file_info: dict[str, Any] | None = Field(None, description="文件信息")

    # 媒体文件
    image_url: str | None = Field(None, description="图片预览 URL（已废弃，请使用 media_url）")
...
class SearchResponse(BaseModel):
    ...
    original_mode: str | None = Field(None, description="原始请求的检索模式（发生降级时有值）")
    mode_fallback: bool = Field(default=False, description="是否发生了模式降级")
    top_k: int = Field(..., description="请求的返回数量")
    vector_weight: float | None = Field(None, description="向量检索权重（回显入参）")
    bm25_weight: float | None = Field(None, description="BM25 检索权重（回显入参）")
    content_weight: float | None = Field(None, description="内容字段权重（回显入参）")
    question_weight: float | None = Field(None, description="问题字段权重（回显入参）")
    rrf_k: int | None = Field(None, description="RRF 平滑参数（回显入参）")
    score_threshold: float | None = Field(None, description="最低分数阈值（回显入参）")

    # LLM 回答（当 llm.enabled=true 时返回）
    answer: str | None = Field(None, description="LLM 生成的回答（启用 LLM 时返回）")
    answer_model: str | None = Field(None, description="生成回答使用的模型名称")
    answer_elapsed_ms: float | None = Field(None, description="LLM 回答耗时（毫秒）")

    # 性能指标
    elapsed_ms: float | None = Field(None, description="检索耗时（毫秒）")
