...
from novamind.features.knowledge_space.integrations.deepdoc.logging_compat import get_logger
from novamind.features.knowledge_space.integrations.deepdoc.core.models import DeepDocParseResult
from novamind.features.knowledge_space.integrations.deepdoc.vision_runtime import ensure_vision_parser_available
...
class DeepDocParser:
    ...
    def _docx_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.docx import RAGFlowDocxParser

        return RAGFlowDocxParser()

    @cached_property
    def _epub_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.epub import RAGFlowEpubParser

        return RAGFlowEpubParser()

    @cached_property
    def _excel_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.excel import RAGFlowExcelParser

        return RAGFlowExcelParser()

    @cached_property
    def _figure_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.figure import RAGFlowFigureParser

        return RAGFlowFigureParser()

    @cached_property
    def _docling_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.remote.docling import RAGFlowDoclingParser

        return RAGFlowDoclingParser()

    @cached_property
    def _mineru_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.remote.mineru import RAGFlowMinerUParser

        return RAGFlowMinerUParser()

    @cached_property
    def _opendataloader_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.remote.opendataloader import RAGFlowOpenDataLoaderParser

        return RAGFlowOpenDataLoaderParser()

    @cached_property
    def _paddleocr_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.remote.paddleocr import RAGFlowPaddleOCRParser

        return RAGFlowPaddleOCRParser()

    @cached_property
    def _somark_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.remote.somark import RAGFlowSoMarkParser

        return RAGFlowSoMarkParser()

    @cached_property
    def _tcadp_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.remote.tcadp import RAGFlowTCADPParser
    ...
    def _ppt_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.ppt import RAGFlowPptParser

        return RAGFlowPptParser()

    @cached_property
    def _text_parser(self):
        from novamind.features.knowledge_space.integrations.deepdoc.parsers.text import RAGFlowTextParser
