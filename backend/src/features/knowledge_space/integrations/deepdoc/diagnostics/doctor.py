...
def build_remediation(
    *,
    ...
    if missing_model_groups:
        next_steps.append(
            "Download vision model groups with `python -m novamind.features.knowledge_space.integrations.deepdoc prepare`"
    ...
    if not text_concat_model_status["available"]:
        next_steps.append(
            "Download the text-concat model with `python -m novamind.features.knowledge_space.integrations.deepdoc prepare --include-text-concat`"
        )
    if not next_steps:
