...
class RAGFlowSoMarkParser(RAGFlowPdfParser):
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = (base_url or os.getenv("SOMARK_BASE_URL", "")).strip().rstrip("/")
        ...
        self.api_key = api_key if api_key else os.getenv("SOMARK_API_KEY", "")
    ...
    def parse_bytes(
        ...
        *,
        ...
        original_formats = dict(self.element_formats)
        original_features = dict(self.feature_config)
        self.element_formats.update({
            "image": str(parsing_config.get("somark_image_format", self.element_formats.get("image", "url"))).strip().lower(),
            "formula": str(parsing_config.get("somark_formula_format", self.element_formats.get("formula", "latex"))).strip().lower(),
            "table": str(parsing_config.get("somark_table_format", self.element_formats.get("table", "html"))).strip().lower(),
            "cs": str(parsing_config.get("somark_cs_format", self.element_formats.get("cs", "image"))).strip().lower(),
        })
        for key in (
            ...
            config_key = f"somark_{key}"
            if config_key in parsing_config:
                self.feature_config[key] = bool(parsing_config[config_key])

        try:
            sections, tables = self.parse_pdf(
                filepath=file_name,
                binary=file_bytes,
                parse_method=str(parsing_config.get("somark_output_mode", "raw")),
            )
        finally:
            self.element_formats = original_formats
            self.feature_config = original_features
        ...
        metadata: dict[str, Any] = {
            ...
            "element_formats": dict(self.element_formats),
