...
class RAGFlowMinerUParser(RAGFlowPdfParser):
    def __init__(self, mineru_path: str = "mineru", mineru_api: str = "", mineru_server_url: str = ""):
        self.mineru_path = mineru_path
        self.mineru_api = (mineru_api or os.getenv("MINERU_APISERVER", "")).rstrip("/")
        self.mineru_server_url = (mineru_server_url or os.getenv("MINERU_SERVER_URL", "")).rstrip("/")
        self.outlines = []
        self.logger = logging.getLogger(self.__class__.__name__)

    def is_configured(self) -> bool:
        return bool(self.mineru_api)
    ...
    def parse_bytes(
        ...
        *,
        ...
        sections, tables = self.parse_pdf(
            filepath=file_name,
            binary=file_bytes,
            backend=str(parsing_config.get("mineru_backend", "pipeline")),
            server_url=parsing_config.get("mineru_server_url") or self.mineru_server_url or None,
            delete_output=bool(parsing_config.get("mineru_delete_output", True)),
            parse_method=str(parsing_config.get("mineru_output_mode", "raw")),
            parser_config=parsing_config,
        )
        ...
        metadata: dict[str, Any] = {
            ...
            "service": {
                "api_url": self.mineru_api,
                "server_url": self.mineru_server_url,
                "configured": bool(self.mineru_api),
            },
            "backend": str(parsing_config.get("mineru_backend", "pipeline")),
            "tables": tables,
