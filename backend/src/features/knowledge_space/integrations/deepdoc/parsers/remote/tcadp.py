...
try:
    ...
    TENCENTCLOUD_SDK_AVAILABLE = True
except ImportError:
    credential = None
    ClientProfile = None
    HttpProfile = None
    ...
    class TencentCloudSDKException(Exception):
        """Fallback exception used when Tencent Cloud SDK is unavailable."""
...
from novamind.features.knowledge_space.integrations.deepdoc.parsers.upstream.utils import extract_pdf_outlines
...
class TencentCloudAPIClient:
    ...
    def __init__(self, secret_id, secret_key, region):
        if not TENCENTCLOUD_SDK_AVAILABLE:
            raise RuntimeError(
                "[TCADP] Tencent Cloud SDK is not installed. Install package 'tencentcloud-sdk-python'."
            )
...
class RAGFlowTCADPParser(RAGFlowPdfParser):
    def __init__(self, secret_id: str = None, secret_key: str = None, region: str = "ap-guangzhou", table_result_type: str = None, markdown_image_response_type: str = None):
        ...
        self.secret_id = secret_id or os.getenv("TCADP_SECRET_ID") or os.getenv("TENCENTCLOUD_SECRET_ID")
        self.secret_key = secret_key or os.getenv("TCADP_SECRET_KEY") or os.getenv("TENCENTCLOUD_SECRET_KEY")
        self.region = region or os.getenv("TCADP_REGION", "ap-guangzhou")
        self.table_result_type = table_result_type if table_result_type is not None else os.getenv("TCADP_TABLE_RESULT_TYPE", "1")
        self.markdown_image_response_type = markdown_image_response_type if markdown_image_response_type is not None else os.getenv("TCADP_MARKDOWN_IMAGE_RESPONSE_TYPE", "1")
    ...
    def is_configured(self) -> bool:
        return bool(TENCENTCLOUD_SDK_AVAILABLE and self.secret_id and self.secret_key)
    ...
    def parse_bytes(
        ...
        *,
        ...
        if not self.secret_id or not self.secret_key:
            raise RuntimeError("[TCADP] TCADP credentials are not configured.")

        parsing_config = parsing_config or {}
        original_table_result_type = self.table_result_type
        original_markdown_image_response_type = self.markdown_image_response_type
        if "tcadp_table_result_type" in parsing_config:
            self.table_result_type = str(parsing_config["tcadp_table_result_type"])
        if "tcadp_markdown_image_response_type" in parsing_config:
            self.markdown_image_response_type = str(parsing_config["tcadp_markdown_image_response_type"])

        try:
            sections, tables = self.parse_pdf(
                filepath=file_name,
                binary=file_bytes,
                file_type=str(parsing_config.get("tcadp_file_type", "PDF")),
                file_start_page=int(parsing_config.get("tcadp_file_start_page", 1)),
                file_end_page=int(parsing_config.get("tcadp_file_end_page", 1000)),
                delete_output=bool(parsing_config.get("tcadp_delete_output", True)),
                max_retries=int(parsing_config.get("tcadp_max_retries", 1)),
            )
        finally:
            self.table_result_type = original_table_result_type
            self.markdown_image_response_type = original_markdown_image_response_type
        ...
        for item in sections:
            ...
            if chunk:
                default_chunks.append(chunk)

        full_text = "\n\n".join(default_chunks).strip()
        metadata: dict[str, Any] = {
            ...
            "service": {
                "region": self.region,
                "configured": bool(TENCENTCLOUD_SDK_AVAILABLE and self.secret_id and self.secret_key),
                "sdk_available": TENCENTCLOUD_SDK_AVAILABLE,
            },
            "table_result_type": self.table_result_type,
            "markdown_image_response_type": self.markdown_image_response_type,
            "tables": tables,
            "section_count": len(default_chunks),
        }
        return full_text, default_chunks or ([full_text] if full_text else []), metadata

    def check_installation(self) -> bool:
        """Check if Tencent Cloud API configuration is correct"""
        try:
            if not TENCENTCLOUD_SDK_AVAILABLE:
                self.logger.error("[TCADP] Tencent Cloud SDK is not installed")
                return False
            if not self.secret_id or not self.secret_key:
