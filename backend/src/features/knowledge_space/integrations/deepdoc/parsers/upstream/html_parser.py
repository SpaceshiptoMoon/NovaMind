...
from novamind.features.knowledge_space.integrations.deepdoc.compat import find_codec, rag_tokenizer
import logging
import re
import uuid
...
try:
    import chardet
except ImportError:  # Optional; local codec detection covers the fallback path.
    chardet = None
...
def get_encoding(file):
    ...
    if chardet is not None:
        detected = chardet.detect(binary).get("encoding")
        if detected:
            return detected
    return find_codec(binary)
