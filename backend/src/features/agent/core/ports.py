...
本模块定义 Agent 引擎（`agent/core/*`）对宿主能力的抽象依赖：
  - WebSearchPort：联网搜索（替代直接 import deep_research.duckduckgo_service）
  - KnowledgeSearchPort：知识空间/知识库发现与文档检索（替代直接 import
    knowledge_space.repository/services 与 user.services）
  - MemoryStorePort：长期记忆 MySQL 持久化 + 上下文压缩摘要（替代直接 import
    agent.repository.memory_repository/context_summary_repository 与 AgentMemory ORM）
  - MemorySearchPort：长期记忆 ES 向量检索（替代直接 import
    agent.repository.memory_search_repository 与 shared.clients.ClientFactory）

设计约束（与 `shared/engine_ports.py` 一致）：
  - 协议只描述引擎所需能力，不携带 ORM/枚举/配置键等业务实体。
  - 引擎构造器/工具经 `context` 字典接收这些协议的实例；宿主在装配时注入实现
    （`features/agent/adapters/`）。
  - 依赖方向：宿主 -> 引擎 -> 本协议；引擎 ✗-> 宿主 features/setting/shared.clients。
...
if TYPE_CHECKING:
    ...
    # 避免顶部 import 触发 memory 包 __init__ 的循环导入
    # （ports ← memory_manager ← long_term ← ports）。
