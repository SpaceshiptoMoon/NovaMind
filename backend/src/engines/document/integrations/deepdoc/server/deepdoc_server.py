...
from novamind.engines.document.integrations.deepdoc.server.endpoints import (
    create_dla_router,
...
def create_deepdoc_app(engine: Optional[DeepDocEngine] = None) -> FastAPI:
    ...
    try:
        from novamind.engines.document.integrations.deepdoc.server.adapters import DLAAdapter
    ...
    try:
        from novamind.engines.document.integrations.deepdoc.server.adapters import OCRAdapter
    ...
    try:
        from novamind.engines.document.integrations.deepdoc.server.adapters import TSRAdapter
