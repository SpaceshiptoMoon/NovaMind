...
class DeepDocParser:
    ...
    def __init__(self):
        self._docx_parser = RAGFlowDocxParser()
        self._epub_parser = RAGFlowEpubParser()
        self._excel_parser = RAGFlowExcelParser()
        self._pdf_parser = RAGFlowPdfParser()
        self._ppt_parser = RAGFlowPptParser()
        self._text_parser = RAGFlowTextParser()

    @staticmethod
    def supported_extensions() -> set[str]:
        return {"pdf", "docx", "epub", "txt", "md", "markdown", "csv", "json", "html", "xls", "xlsx", "ppt", "pptx"}
    ...
    def _parse_pdf_sync(
        ...
        # Keep our current structured extraction layer, but also attach the vendored
        # RAGFlow plain-parser outlines and plain sections so we can iterate toward
        # fuller upstream compatibility without blocking the runtime path.
    ...
    def _parse_text_sync(
        ...
        if isinstance(source, Path):
            full_text, default_chunks, parser_metadata = self._text_parser.parse(
                source,
                parser_id=parsing_config.get("deepdoc_parser_id"),
            )
            file_type = source.suffix.lower().lstrip(".")
        else:
            full_text, default_chunks, parser_metadata = self._text_parser.parse_bytes(
                source,
                extension,
                parser_id=parsing_config.get("deepdoc_parser_id"),
            )
