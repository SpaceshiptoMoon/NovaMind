...
def test_deepdoc_cli_capabilities():
    completed = subprocess.run(
        [sys.executable, "-m", "novamind.features.knowledge_space.integrations.deepdoc", "capabilities"],
        cwd=BACKEND_ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def test_deepdoc_cli_doctor():
    completed = subprocess.run(
        [sys.executable, "-m", "novamind.features.knowledge_space.integrations.deepdoc", "doctor"],
        cwd=BACKEND_ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def test_deepdoc_cli_doctor_smoke(monkeypatch, capsys):
    from novamind.features.knowledge_space.integrations.deepdoc import __main__ as deepdoc_cli
...
def test_deepdoc_server_factory_importable():
    ...
    factory = import_from_string("novamind.features.knowledge_space.integrations.deepdoc.server.deepdoc_server:create_deepdoc_app")
    app = factory()
