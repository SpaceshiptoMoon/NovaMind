...
def test_deepdoc_cli_capabilities():
    completed = subprocess.run(
        [sys.executable, "-m", "novamind.engines.document.integrations.deepdoc", "capabilities"],
        cwd=BACKEND_ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def test_deepdoc_cli_doctor():
    completed = subprocess.run(
        [sys.executable, "-m", "novamind.engines.document.integrations.deepdoc", "doctor"],
        cwd=BACKEND_ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def test_deepdoc_cli_doctor_smoke(monkeypatch, capsys):
    from novamind.engines.document.integrations.deepdoc import __main__ as deepdoc_cli
