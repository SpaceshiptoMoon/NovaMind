...
> 背景：见 [`IMPROVEMENT-enterprise-kb.md`](./IMPROVEMENT-enterprise-kb.md) §2.1 —— 重构前 `qa` 模块是「无根对话」（纯上下文拼接 + 裸生成，**完全不调知识库检索**）。本系列重构（批次 1–4）把它升级为完整的 RAG 管道：**查询改写 → 检索增强 → Grade→Retry 自评估 → 检索链路 trace**。
