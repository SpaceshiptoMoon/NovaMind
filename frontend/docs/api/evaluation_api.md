...
> **说明**：API 响应中的 `status` 字段统一返回字符串值（如 `"completed"`），不再返回整数。
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| skip | int | 否 | `0` | 跳过的记录数（≥0） |
| limit | int | 否 | `20` | 返回的最大记录数（1-100） |
...
删除指定的测试集。若有关联的待执行（pending）或执行中（running）任务则拒绝删除。删除时会同时清理 MinIO 中的测试集文件及关联任务的结果文件。
...
| top_k | int | `5` | 检索返回数量（1-50） |
| score_threshold | float | `0.0` | 检索分数阈值（0.0-1.0） |
...
| skip | int | 否 | `0` | 跳过的记录数（≥0） |
| limit | int | 否 | `20` | 返回的最大记录数（1-100） |
| status | int | 否 | 无 | 按状态过滤，可选值：`1`（pending）、`2`（completed）、`3`（failed）、`5`（running）、`6`（cancelled） |
...
删除指定的测评任务。待执行（pending）或执行中（running）的任务不可删除，删除时会同时清理 MinIO 中的结果文件。
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| scores | array | 是 | 评分列表（最多 500 条） |
| scores[].index | int | 是 | 测试用例索引（从 0 开始） |
| scores[].score | int | 是 | 人工评分（1-10） |
| scores[].comment | string | 否 | 评语（最大 1000 字符） |
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| format | string | 否 | `json` | 导出格式，仅支持 `json` 或 `csv`（其他值返回 422） |
