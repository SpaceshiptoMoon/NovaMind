...
> **更新 (2026-07)**：以下 8 个严重问题和 6 个缺失功能已全部修复/实现，状态标注如下。代码质量问题部分已核实当前状态。
...
| # | 问题 | 当前状态 |
|---|------|---------|
| 1 | 6 个通用组件已编写但从未被任何页面使用 | ❌ 待核实 — 部分组件（如 Pagination、StatusTag、SearchBar）已被页面使用，需逐组件核实 |
| 2 | `MainLayout` 侧边栏无法高亮嵌套路由（`/spaces/1/knowledge-bases` 不会激活"知识空间"菜单） | ❌ 待核实 — 需检查 `MainLayout.vue` 中 `route.matched` 逻辑 |
| 3 | 无 `.env` 文件，无 Vite 开发代理配置，无法联调后端 | ✅ 已修复 — `.env.development`、`.env.production`、`.env.example` 均已存在，`vite.config.ts` 已配置代理 |
| 4 | 多处 `ElMessageBox` 取消判断使用不安全的 `(error as string) !== 'cancel'` | ⚠️ 部分修复 — 需逐文件检查是否仍有此类写法 |
| 5 | vitest 已配置但无任何测试文件 | ❌ 仍未解决 — `src/` 下无任何 `.test.ts` 或 `.spec.ts` 文件 |
| 6 | 路由守卫直接读 `localStorage` 而非使用 user store | ❌ 待核实 — 需检查 `router/guards.ts` 当前实现 |

---

## 开发计划

> **说明**：以下 Stage 0-9 为 2026-05 制定的开发计划，当前项目已基本完成大部分阶段。其中 Stage 0-4（基础设施/API/Store/布局/用户管理）、Stage 6（AI 对话）、Stage 7（深度研究）已基本实现；Stage 5（知识空间高级搜索配置）和 Stage 8-9（组件统一/测试）仍有部分未完成。以下计划保留作为参考，当前应优先关注代码质量问题的修复和测试覆盖。
